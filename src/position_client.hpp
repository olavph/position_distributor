...
void PositionClient::connect() {
  ...
  m_resolver.async_resolve(
      m_host, port_str, beast::bind_front_handler(&PositionClient::on_resolve, shared_from_this()));
}
...
void PositionClient::on_connect(beast::error_code ec,
  ...
  std::println("Connected to: {}", m_host);
...
void PositionClient::on_handshake(beast::error_code ec) {
  ...
  std::println("Sent Client ID: {}", m_client_id);
}
...
void PositionClient::on_read(beast::error_code ec, std::size_t bytes_transferred) {
  ...
  std::println("Received position from client {}: {} = {}", client_id, position.m_symbol,
